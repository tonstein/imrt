...
public:
  ImEditor(std::string title, ImVec2 windowSize, float fontSize,
           ...
           bool alwaysOnTop)
      : _title(title), _windowSize(windowSize), _fontSize(fontSize) {
    ...
    _window = glfwCreateWindow(windowSize.x, windowSize.y, title.c_str(),
                               nullptr, nullptr);
    if (_window == NULL)
      std::exit(1);
...
private:
  void onStart() { static_cast<Derived *>(this)->onStart(); }

  void onUpdate() { static_cast<Derived *>(this)->onUpdate(); }
